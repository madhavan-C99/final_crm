...
// ---- Moved outside LeadRowDetails so these don't get re-created
// (and their subtree re-mounted) on every render of the parent. ----
...
// About Lead (ALL remarks) + Timeline.
const LeadRowDetails = ({ leadId }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        // Sequential axios calls — lead details first, then call
        // history.
  ...
  // remarks with a note, most recent first, each with its own date/time
  ...
  return (
    ...
    >
      <Box sx={{ width: "100%", maxWidth: "1380px" }}>
        <Grid container spacing={2} alignItems="flex-start">
          ...
          <Grid size={{ xs: 12, md: 7 }}>
            <Box sx={scrollableCardSx}>
              <Typography
                sx={{ fontWeight: 700, fontSize: "15px", mb: "14px" }}
              >
                Timeline
              </Typography>
