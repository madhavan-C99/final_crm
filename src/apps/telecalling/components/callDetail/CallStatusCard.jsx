...
const CallStatusCard = ({
  ...
  const hasOtherActiveCall =
    activeCallLeadId &&
    activeCallLeadId !== String(leadId);
    ...
    >
      ...
      >
        ...
        {hasOtherActiveCall && (
          ...
          >
            <Box>
              <Typography sx={{ fontWeight: 700, fontSize: "14px", color: "#D48806" }}>
                Unsaved Call Data Pending!
              </Typography>
              <Typography sx={{ fontSize: "13px", color: "#595959", mt: 0.5 }}>
                Last call data for <strong>{activeCallLeadName || "previous lead"}</strong> has not been saved yet. Please save that call data first before connecting to another person.
              </Typography>
            </Box>
            <Button
              variant="contained"
              size="small"
              onClick={() => navigate(`/lead-details/${activeCallLeadId}`)}
