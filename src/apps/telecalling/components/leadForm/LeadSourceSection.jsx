...
const LeadSourceSection = ({
    ...
    useEffect(() => {
        ...
        const selected = leadSourceOptions.find(
            item => item.label.trim().toLowerCase() === leadData.lead_source.trim().toLowerCase()
        );
    ...
    useEffect(() => {
        ...
        const selected = sourceTypeOptions.find(
            item => item.label.trim().toLowerCase() === leadData.campaign_name.trim().toLowerCase()
        );
    ...
    const getCourseTimingOptions = async (planId) => {
        try {
            const payload = {
                dropdown_category: "course_time",
                filter_id: planId,
                course_name_id: selectedCourseId
            };

            const response = await getDropdownOptions(payload);
            setCourseTimingOptions(response.data.data || []);
        } catch (error) {
            console.log(error);
        }
    };
                    ...
                    <Box
                        onClick={() => {
                            if (!formData.course_name) {
                                setErrors(prev => ({
                                    ...prev,
                                    course_name: "Please select Course Name",
                                }));
                            }
                        }}
                    >
                        ...
                        <TextField
                            select
                            fullWidth
                            disabled={!formData?.course_name || !isEdit}
                            name="course_plan"
                            value={currentCoursePlanId}
                            onChange={handleCoursePlanChange}
                            error={!!parentErrors?.course_plan || !!errors.course_name}
                            helperText={parentErrors?.course_plan || errors.course_name}
                            sx={fieldStyle}
                        >
                            {coursePlanOptions.map((item) => (
